...
- init_usine : Cette fonction initialise un nombre de t_ligne_centrifugeuse
et le nombre de centrifugeuse en fonction dans chaque ligne
- toc_usine : Cette fonction assure que chaque ligne de centrifugeuse
subissent un toc.
- entretien_usine : Cette fonction observe les centrifugeuses de l'usine et
va les remplacer si les conditions de remplacement définie
dans le programme sont valide.
Les centrifugeuse remplacées sont mises dans le tableau
dynamique poubelle_ligne.
- print_usine : Cette fonction affiche les statistique et lignes de l'usine.
- get_nb_actuel_en_bris : Cette fonction donne le nombre de centrifigeuse en
bris dans l'usine.
- get_nb_actuel_en_fonction : Cette fonction donne le nombre de centrifigeuse
en fonction dans l'usine.
- get_nb_toc : Cette fonction donne le nombre de toc depuis le démarage
de l'usine.
- get_nb_bris_total : Cette fonction donne le nombre bris depuis le
démarage de l'usine.
...
- nb_fonction : le nombre de centrifugeuse initialisées en fonction
dans chaque ligne (type:uint)

VALEUR DE RETOUR : 1 si réussi, 0 sinon (int).
*/
...
centrifugeuse à mettre en fonction(peu importe sa ligne).

PARAMETRE : -ptr_usine : la structure t_usine définie (type:t_usine*)
- temps : la variable temps (type:int)
...
VALEUR DE RETOUR : Nombre de centrifeuse qui sont remplacées (int)
*/
...
VALEUR DE RETOUR : Nombre de centrifeuse en fonction dans l'usine (int)
*/
...
VALEUR DE RETOUR : Nombre de =toc depuis l'initialisation de l'usine(type:int)
*/
...
VALEUR DE RETOUR : Nombre de bris depuis l'initialisation de l'usine(int)
*/
