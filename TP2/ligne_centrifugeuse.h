...
- reduire_cnt : Cette fonction réduit de un le nombre de  centrifugeuses EN_FONCTION
dans la ligne.
- toc_ligne : Cette fonction déclenche la fonction toc_centrifugeuse() pour
...
remplace celle à la position donnée dans le tableau.
- get_en_etat : La fonction retourne le train de bits de la ligne qui donne les
positions des centrifugeuses dans l'état choisi
- get_centrifugeuse : La fonction retourne une copie de la centrifugeuse à cette
...
- set_config : Donne la chaine de bits correspondant à létat demander
- configuration_valide : fonction configuration_valide du TP1
- t_centrifugeuse : La fonction privée du module permute  les centrifugeuses aux
...
PARAMETRE : -ptr_lig : Prend les données issu de la structure t_ligne_centrifugeuse
type(t_ligne_centrifugeuse*)
- temps : variable temps définissant le temps de reparation restant (type:int)
*/
...
PARAMETRE : -ptr_lig : Prend les données issu de la structure t_ligne_centrifugeuse
(type: t_ligne_centrifugeuse*)
- état : Le second paramètre doit être une des quatre constantes (type:int)
...
PARAMETRE : -ptr_lig : Prend les données issu de la structure 
...
PARAMETRE : -ptr_lig : Prend les données issu de la structure t_ligne_centrifugeuse
(type: t_ligne_centrifugeuse)
- etat : L'état voulu (EN_BRIS, EN_ARRET, EN_ATTENTE, EN_FONCTION)
...
La fonction privée du module cree et retourne une centrifugueuses dont tout
